...
let presets.comments =  Syntax({'fg': '238'})
let presets.functions = Syntax({'fg': '026'})
let presets.storage = Syntax({'fg': '099'})
let presets.variables = Syntax({'fg': '099'})
let presets.strings = Syntax({'fg': '149'})
...
let syntax.Directory = copy(presets.strings)
let syntax.NERDTreeDirSlash = copy(presets.comments)
let syntax.NERDTreeFile = copy(presets.comments)
...
let syntax.Function = copy(presets.functions)
...
let syntax.jsFuncCall = copy(presets.functions)
let syntax.jsonKeyword = copy(presets.storage)
let syntax.jsFunctionKey = copy(presets.functions)
...
let syntax.Noise  = copy(presets.comments)
